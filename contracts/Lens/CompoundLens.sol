...
contract CompoundLens {
    ...
    function getVoteInfo(address _user,address _urz,address _wurz) public view returns(uint256 urzBalance,uint256 surplusVotes,uint256 totalVote,uint256 castVote){
        urzBalance = ITRC20(_urz).balanceOf(_user);
        surplusVotes = ITRC20(_wurz).balanceOf(_user);
        castVote = IWURZ(_wurz).lockOf(_user);
        totalVote = add(surplusVotes,castVote,"sum vote total");
    }
