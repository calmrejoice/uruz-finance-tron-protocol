...
contract GovernorAlpha {
    ...
    string public constant name = "Uruz Governor Alpha";

    /// @notice The number of votes in support of a proposal required in order for a quorum to be reached and for a vote to succeed
    function quorumVotes() public pure returns (uint) {return 400000000e18;} // 400,000,000 = 4% of URZ(10,000,000,000)

    /// @notice The number of votes required in order for a voter to become a proposer
    function proposalThreshold() public pure returns (uint) {return 100000000e18;} // 100,000,000 = 1% of URZ(10,000,000,000)
    ...
    constructor(address wurz_, address guardian_) public {
        wurz = WURZInterface(wurz_);
        guardian = guardian_;
    }
...
//    }
    ...
    function propose(address[] memory targets, uint[] memory values, string[] memory signatures, bytes[] memory calldatas, string memory description) public returns (uint) {
        require(wurz.getPriorVotes(msg.sender, sub256(block.number, 1)) > proposalThreshold(), "GovernorAlpha::propose: proposer votes below proposal threshold");
    ...
    function cancel(uint proposalId) public {
        ...
        Proposal storage proposal = proposals[proposalId];
        require(msg.sender == guardian || wurz.getPriorVotes(proposal.proposer, sub256(block.number, 1)) < proposalThreshold(), "GovernorAlpha::cancel: proposer above threshold");
    ...
    function _castVote(address voter, uint proposalId, uint votes, bool support) internal {
        ...
        //uint96 votes = wurz.getPriorVotes(voter, proposal.startBlock);
        require(votes <= wurz.getPriorVotes(voter, proposal.startBlock), "GovernorAlpha::_castVote: short of vote power");
        if (support) {
        ...
        // burn 
        require(wurz.voteFresh(voter, proposalId, support, votes), "wurz vote exception");
